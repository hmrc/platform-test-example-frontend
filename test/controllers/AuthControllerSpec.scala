  ...
  "signOut" - {
    ...
    "must clear user answers and redirect to sign out, specifying the exit survey as the continue URL" in {
      ...
      running(application) {
        ...
        val expectedRedirectUrl = s"http://localhost:1066/platform-test-example-frontend?continue=http%3A%2F%2Flocalhost%3A9514%2Ffeedback%2Fplatform-test-example-frontend"
  ...
  "signOutNoSurvey" - {
    ...
    "must clear users answers and redirect to sign out, specifying SignedOut as the continue URL" in {
      ...
      running(application) {
        ...
        val expectedRedirectUrl = s"http://localhost:1066/platform-test-example-frontend?continue=%2Fplatform-test-example-frontend%2Faccount%2Fsigned-out"
